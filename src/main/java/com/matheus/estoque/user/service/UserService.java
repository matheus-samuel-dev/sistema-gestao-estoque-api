...
import com.matheus.estoque.user.entity.AccountStatus;
...
public class UserService {
    ...
    public AuthResponseDTO googleLogin(String googleToken) {
        Map<String, Object> googleUser = loadGoogleUser(googleToken);

        if (googleUser == null) {
            throw new RuntimeException("Token Google inválido");
        }

        String email = String.valueOf(googleUser.getOrDefault("email", ""));
        String name = String.valueOf(googleUser.getOrDefault("name", email));
        Object verifiedValue = googleUser.get("email_verified");
        boolean emailVerified = verifiedValue == null || Boolean.parseBoolean(String.valueOf(verifiedValue));

        if (email.isBlank() || !emailVerified) {
            throw new RuntimeException("Conta Google inválida ou e-mail não verificado");
        }
    ...
    private Map<String, Object> loadGoogleUser(String googleToken) {
        try {
            ...
            return restTemplate.getForObject(
                    "https://oauth2.googleapis.com/tokeninfo?id_token=" + encodedToken,
                    Map.class
            );
        }
    }
